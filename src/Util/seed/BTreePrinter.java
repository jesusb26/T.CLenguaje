...
public class BTreePrinter {
    ...
    public static <T extends Comparable<?>> void printNode(NodoB root) {
        int maxLevel = BTreePrinter.maxLevel(root);

        printNodeInternal(Collections.singletonList(root), 1, maxLevel);
    }

    private static <T extends Comparable<?>> void printNodeInternal(List<NodoB> nodes, int level, int maxLevel) {
        ...
        List<NodoB> newNodes = new ArrayList<>();
        for (NodoB node : nodes) {
            if (node != null) {
    ...
    private static <T extends Comparable<?>> int maxLevel(NodoB node) {
        if (node == null)
            return 0;
